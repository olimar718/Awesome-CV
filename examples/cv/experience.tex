...
\begin{cventries}
	...
	{Akkodis (en mission pour Thales)} % Organisation
	...
	{Octobre 2025 - Présent} % Dates
	{
		...
		\item{
			...
			\begin{itemize}
				...
				\item{Gestion de la représentation binaire des types primitifs (nombres \textbf{flottants}, signés, etc.).}
				\item{Génération de code \textbf{C++} à partir d'un DSL via \textbf{Xtend}.}
				\item{Test unitaires \textbf{GoogleTest} et tests d'intégration.}
			\end{itemize}
		}
		\item{
	...
	{Février 2024 - Octobre 2025} % Dates
	{
		...
		\item{
			...
			\begin{itemize}
				...
				\item{\textbf{Intégration} de code source \textbf{C++} fournis par une autre équipe : validation de l'impact du changement d'environnement}
			\end{itemize}
		}
		\item{
			...
			\begin{itemize}
				...
				\item{Écriture d'un générateur de code \textbf{C} en Python pour la sérialisation/désérialisation. \textbf{C++} : Génération de tests unitaires aléatoires pour le code généré}	
				\item{Scripts \textbf{bash} très robustes pour la synchronisation temporelle \textbf{NTP}}
			\end{itemize}
		}
		\end{cvitems}
	}
